...
```{r plot, cache=FALSE, fig.width=8, fig.height=3, fig.cap="**Accuracy of approxmating $\\beta$ from z-statistics.** **A**) Sample size is varied with $\\beta=0$, $\\phi=0.5$ **B**) Case ratio is varied with $\\beta=0$, $n=10,000$ **C**) $\\beta$ is varied with $\\phi=0.5$, $n=10,000$"}
